...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(displaySensorReading_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    displaySensorReading_step();
...
{
  UNUSED(sig);
  rtmSetErrorStatus(displaySensorReading_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  displaySensorReading_terminate();
  sem_post(&stopSem);
  return NULL;
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(displaySensorReading_M, 0);

  /* Initialize model */
  displaySensorReading_initialize();

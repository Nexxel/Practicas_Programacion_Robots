...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(collAvoidLogic_M) == (NULL));
  while (runModel) {
...
{
  UNUSED(sig);
  rtmSetErrorStatus(collAvoidLogic_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(collAvoidLogic_M, 0);

  /* Initialize model */
  collAvoidLogic_initialize();

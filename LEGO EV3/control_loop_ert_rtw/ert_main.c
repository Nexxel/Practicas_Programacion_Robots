...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(control_loop_M) == (NULL)) &&
    !rtmGetStopRequested(control_loop_M);
  while (runModel) {
...
{
  UNUSED(sig);
  rtmSetErrorStatus(control_loop_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(control_loop_M, 0);

 ...
 * C/C++ source code generated on : Wed Apr 25 14:22:12 2018
 *
...
void control_light_step(void)
{
  ...
  /* '<S2>:1:4' if isempty(times) */
  /*      if(current_time < 5) */
  /*  times = [times current_time]; intensities = [intensities color_intensity]; */
  /* '<S2>:1:10' ended = 0; */
  /* '<S2>:1:11' R = 55; */
  /* '<S2>:1:12' U = 5; */
  /* '<S2>:1:13' P = 20; */
  /* '<S2>:1:14' if color_intensity < R - U */
  if (tmp < 50) {
    /* '<S2>:1:15' left_motor = -P; */
    rtb_left_motor = -20;

    /* '<S2>:1:16' right_motor = P; */
    rtb_right_motor = 20;
  } else if (tmp > 60) {
    /* '<S2>:1:17' elseif color_intensity > R + U */
    /* '<S2>:1:18' left_motor = P; */
    rtb_left_motor = 20;

    /* '<S2>:1:19' right_motor = -P; */
    rtb_right_motor = -20;
  } else {
    /* '<S2>:1:20' else */
    /* '<S2>:1:21' left_motor = P; */
    rtb_left_motor = 20;

    /* '<S2>:1:22' right_motor = P; */
    rtb_right_motor = 20;
  }
   ...
   */
  /* '<S2>:1:24' pause(10); */
  /*      elseif(current_time == 5) */
  /*          left_motor = 0; */
  /*          right_motor = 0; */
  /*          file = fopen('output.txt','w'); */
  /*          for i = 1:length(times) */
  /*              fprintf(file, '%f \t %f', double(times(1,i)), double(intensities(1,i))); */
  /*          end */
  /*          fclose(file); */
  /*          ended = 1; */
  /*      else */

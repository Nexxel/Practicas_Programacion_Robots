...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(control_bucle_M) == (NULL));
  while (runModel) {
...
{
  UNUSED(sig);
  rtmSetErrorStatus(control_bucle_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(control_bucle_M, 0);

  /* Initialize model */
  control_bucle_initialize();
